...
// ─── OAuth helpers (same as post-pending-tweets) ──────────────────────────────
...
async function postReplyToX(text: string, replyToId: string): Promise<{ success: boolean; tweetId?: string; error?: string }> {
  ...
  if (!resp.ok) {
    return { success: false, error: `${resp.status}: ${data.detail || JSON.stringify(data)}` };
  }
...
serve(async (req) => {
  ...
  try {
    ...
    // ── RATE LIMIT: max 5 comments per day ──────────────────────────────────
    ...
    if ((todayCount ?? 0) >= 5) {
      console.log(`[TREND-COMMENTER] Daily cap reached (${todayCount}/5). Skipping.`);
    ...
    console.log(`[TREND-COMMENTER] Searching Tavily for: ${randomQuery}`);
    ...
    for (const r of results) {
      const url = r.url || "";
      // Match x.com or twitter.com /status/ID patterns
      const match = url.match(/(?:twitter\.com|x\.com)\/([^/]+)\/status\/(\d+)/);
      if (match) {
        const alreadyCommented = await sb
          .from("trend_comment_logs")
          .select("id")
          .eq("tweet_id", match[2])
          .maybeSingle();
        if (!alreadyCommented.data) {
          tweetCandidates.push({
            tweetId: match[2],
            author: match[1],
            content: r.content?.slice(0, 500) || r.title || "viral crypto post",
            url,
          });
        }
      }
    ...
    if (tweetCandidates.length === 0) {
      console.log("[TREND-COMMENTER] No viable tweet candidates found in this search.");
      ...
      const fallbackResp = await fetch("https://api.tavily.com/search", {
        ...
        body: JSON.stringify({
          api_key: TAVILY_API_KEY,
          query: "crypto market analysis 2026 bitcoin price",
          search_depth: "basic",
          max_results: 5,
          days: 2,
        }),
      ...
      for (const r of fallbackData.results ?? []) {
        ...
        if (match) {
          tweetCandidates.push({ tweetId: match[2], author: match[1], content: r.content?.slice(0, 500) || "crypto post", url: r.url });
        }
      }
    }
    ...
    console.log(`[TREND-COMMENTER] Target: @${target.author} tweet ${target.tweetId}`);

    // ── GENERATE COMMENT WITH AI ─────────────────────────────────────────────
    let comment = "";
    
    // Try Claude first (premium)
    ...
    console.log(`[TREND-COMMENTER] Generated: "${comment.slice(0, 80)}..."`);
    ...
    await sb.from("trend_comment_logs").insert({
      ...
      tweet_author: target.author,
      original_content: target.content.slice(0, 500),
      our_comment: comment,
      x_url: xUrl,
    ...
    if (postResult.success) {
      await sb.from("agent_logs").insert({
        message: `[GROWTH]: Viral comment posted on @${target.author}'s tweet. ${xUrl || ""}`,
      });
      console.log(`[TREND-COMMENTER] SUCCESS. Our tweet: ${xUrl}`);
    } else {
      console.error(`[TREND-COMMENTER] X post failed: ${postResult.error}`);
      await sb.from("agent_logs").insert({
        message: `[GROWTH ERROR]: Trend comment failed for @${target.author}: ${postResult.error}`,
      });
    }
